...
* Test types and parameters
  ...
  <pre>
  Exec           The client will create a 'session' and do an 'exec'. Thus stdin, stdout and stderr are streams which 
                 can be tested.
                 The parameters are in specified as <stream>:<generator|verifier> 

  o:sODV         Verify lines read from stdout via the 'streamOnDataVerify' algorithm.
  e:sODV         As above but from stderr.

  o:wGD          Write lines to stdout generated via 'writeGeneratedData; algorithm.
  e:wGD          As above but to stderr. 
  ...
  ** stdin is not supported yet
  ** data flow is unidirectional at this moment. This the client must verify the data generated by the server.
